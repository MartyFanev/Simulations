...
// set up the initial values of the solution and put them into a file called input.
  ...
  for(int i=0; i < cells+4; i++){ // changed from v.cols to cells+2
      ...
      level_set(i,j) = 0.2 - std::sqrt(pow((x-0.6),2)+pow((y-0.3),2));
...
MatrixXd Solver::ghost_boundary(MatrixXd k){
  ...
  for (int i =2 ; i<cells+3;i++){
    for(int j=2;j<cells+3;j++){
      ...
      if(level_set(i,j)*level_set(i+1,j) < 0 || level_set(i,j)*level_set(i,j+1) < 0){
        //std::cout << level_set(i,j) << std::endl;
        double x = x0 + (i-0.5)*dx;
        double y = y0 + (j-0.5)*dy;
        ...
        double x_centre = x0 + (p_i-0.5)*dx;
        double y_centre = y0 + (p_j-0.5)*dy;

    for (int k =0;k<4;k++){
        if (point_1_x > x_centre && point_1_y > y_centre){
          //std::cout << v(p_i,p_j,0) << " " << v(p_i+1,p_j+1,0)  << " " << v(p_i+1,p_j,0) << " " << v(p_i,p_j+1,0) << " "<< "tr" << " " <<  "\n";


          double interpolated_x = (((x_centre+dx)-point_1_x)*v(p_i,p_j+1,k)/dx) + ((point_1_x-x_centre)*v(p_i+1,p_j+1,k)/dx);
          double interpolated_y = ((x_centre+dx)-point_1_x)*v(p_i,p_j,k)/dx + (point_1_x-x_centre)*v(p_i+1,p_j,k)/dx;
          

          //std::cout << point_1_x <<" > " << x_centre << "\n";
          //std::cout << interpolated_x << "  = " << ((x_centre+dx)-point_1_x)*v(p_i,p_j+1,k)/dx << " + " << (point_1_x-x_centre)*v(p_i+1,p_j+1,k)/dx << "\n";
          //std::cout << (((x_centre+dx)-point_1_x)*v(p_i,p_j+1,k)/dx) << "\n";
          //std::cout << v(p_i,p_j+1,2) << "\n";
          double interpolated = ((y_centre+dy)-point_1_y)*interpolated_y/dy + (point_1_y-(y_centre))*interpolated_x/dy;
          u_fluid(k) = interpolated;


        }else if (point_1_x > x_centre && point_1_y < y_centre){
         

          double interpolated_x = ((x_centre+dx)-point_1_x)*v(p_i,p_j,k)/dx + (point_1_x-x_centre)*v(p_i+1,p_j,k)/dx;
          double interpolated_y = ((x_centre+dx)-point_1_x)*v(p_i,p_j-1,k)/dx + (point_1_x-x_centre)*v(p_i+1,p_j-1,k)/dx;

          double interpolated = (y_centre-point_1_y)*interpolated_y/dy + (point_1_y-(y_centre-dy))*interpolated_x/dy; 

          u_fluid(k) = interpolated;
        }else if (point_1_x < x_centre && point_1_y < y_centre){
          

          double interpolated_x = ((x_centre)-point_1_x)*v(p_i-1,p_j,k)/dx + (point_1_x-(x_centre-dx))*v(p_i,p_j,k)/dx;
          double interpolated_y = ((x_centre)-point_1_x)*v(p_i-1,p_j-1,k)/dx + (point_1_x-(x_centre-dx))*v(p_i,p_j-1,k)/dx;
          ...
          double interpolated_x = ((x_centre)-point_1_x)*v(p_i-1,p_j+1,k)/dx + (point_1_x-(x_centre-dx))*v(p_i,p_j+1,k)/dx;
          double interpolated_y = ((x_centre)-point_1_x)*v(p_i-1,p_j,k)/dx + (point_1_x-(x_centre-dx))*v(p_i,p_j,k)/dx;
       ...
       double normal_velocity_rigid = -(u_fluid(1)/u_fluid(0))*normal_x + (u_fluid(3)/u_fluid(0))*normal_y;
       double tangential_velocity_x = (u_fluid(1)/u_fluid(0)) - normal_velocity_real*normal_x , tangential_velocity_y = (u_fluid(3)/u_fluid(0)) - normal_velocity_real*normal_y;
// HLLC  Right is the real liquid left is the rigid body
      ...
      double v_star = (pressure_r - pressure_l - u_fluid(1)*(s_l-normal_velocity_rigid) - u_fluid(1)*(s_r-normal_velocity_real))/(u_fluid(0)*(s_l-normal_velocity_rigid) - u_fluid(0)*(s_r-normal_velocity_real));
    
      double density_star = u_fluid(0)*((s_r - normal_velocity_real)/(s_r - v_star));
      double density_velocity_star = star_r(i,j,0)*v_star;
      double energy_star = density_velocity_star * ((u_fluid(2))/u_fluid(0)) + (v_star-normal_velocity_real)*(v_star + ((pressure_r)/(u_fluid(0)*(s_r-normal_velocity_real))));
      double velocity_x_star = (density_velocity_star/density_star)*normal_x + tangential_velocity_x, velocity_y_star = (density_velocity_star/density_star)*normal_y + tangential_velocity_y;
      ...
      std::cout << v(i,j,0) << " "  << v(i,j,1) << " " << v(i,j,2) << " " << v(i,j,3) << "\n";
      }
    }
  }
...
for (int i =2 ; i<cells+3;i++){
    for(int j=2;j<cells+3;j++){
      ...
      if ((level_set(i,j)*level_set(i+1,j) > 0 || level_set(i,j)*level_set(i,j+1) > 0) && level_set(i,j) > 0){
          v(i,j,0) = pow(10,100);
          v(i,j,2) = pow(10,100);
          v(i,j,3) = pow(10,100);
          v(i,j,4) = pow(10,100);
...
// sweeping logic
  ...
  for (int i =2 ; i<cells+3;i++){
    for(int j=2;j<cells+3;j++){
      ...
      if((level_set(i,j)*level_set(i+1,j) > 0) && (level_set(i,j) > 0)){

        double  normal_x = (level_set(i+1,j) - level_set(i-1,j))/(2*dx), normal_y = (level_set(i,j+1) - level_set(i,j-1))/(2*dy);

        double Qx = std::min(v(i-1,j,0),v(i+1,j,0));
        double Qy = std::min(v(i,j-1,0),v(i,j+1,0));

        //std::cout << " " << v(i-1,j,0) << "\n";
        //v(i,j,0) = ((normal_x-normal_y)/(((normal_x/dx)+(normal_y/dy)))*(normal_x-normal_y))*((normal_x*Qx)/(dx) + (normal_y*Qy)/(dy));
        //std::cout <<"value:" << v(i,j,0) << " normal_x:" << normal_x <<" normal_y:"<< normal_y << " First term:"<<((normal_x-normal_y)/(((normal_x/dx)+(normal_y/dy)))*(normal_x-normal_y))<< " Second Term_1:" << ((normal_x*Qx)/(dx)) << " Second Term_2:" << ((normal_y*Qy)/(dy)) <<"\n" << std::endl;
