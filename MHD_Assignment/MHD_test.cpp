...
// set up the initial values of the solution and put them into a file called input.
  ...
  for(int i=0; i < cells+2; i++){ // changed from v.cols to cells+2
      for (int j =0 ; j<cells+2; j++){
        ...
        velocity_y = 0.1*std::sin(2*pi*x)*std::exp(-pow(y,2)/pow(0.1,2));
        velocity_z =0.;
...
Tensor<double,3> Solver::flux_all(Tensor<double,3> v,char option){
    ...
    if(option=='x'){
      ...
      for(int i =0;i<cells+2;i++){
        ...
        v_L(i,j,k) = v(i,j,k) - 0.5*delta(i,j,k)*epsalon(i,j,k)*0;

        v_R(i,j,k) = v(i,j,k) + 0.5*delta(i,j,k)*epsalon(i,j,k)*0;
        }
      }
    }
...
void Solver::simulate(){
  ...
  do{
   ...
   for(int k=0; k<8; k++){
    for(int j=0; j<cells+2; j++){
      v(0,j,k) = v(cells,j,k);
      v(cells+1,j,k) = v(1,j,k);
    }


    for(int i=0; i<cells+2; i++){
      v(i,0,k) = v(i,cells,k);
      v(i,cells+1,k) = v(i,1,k);
    }
   }
   ...
   t = t+dt;
    ...
    for(int k=0; k<8; k++){
     for(int j=0; j<cells+2; j++){
       v_new(0,j,k) = v_new(cells,j,k);
       v_new(cells+1,j,k) = v_new(1,j,k);
     }


     for(int i=0; i<cells+2; i++){
       v_new(i,0,k) = v_new(i,cells,k);
       v_new(i,cells+1,k) = v_new(i,1,k);
     }
    }
